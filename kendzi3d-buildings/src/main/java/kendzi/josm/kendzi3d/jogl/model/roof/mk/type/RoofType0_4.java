...
public class RoofType0_4 extends RoofType0 {
    ...
    public RoofTypeOutput buildRectangleRoof(RectangleRoofTypeConf conf) {
        ...
        Double l3 = getLenghtMetersPersent(conf.getMeasurements(), MeasurementKey.LENGTH_3, conf.getRecHeight(), conf.getRecHeight() / 3d);
        Double l4 = getLenghtMetersPersent(conf.getMeasurements(), MeasurementKey.LENGTH_4, conf.getRecWidth(), conf.getRecWidth() / 3d);

        int type = getType();

        return build(conf.getBuildingPolygon(), conf.getRecHeight(), conf.getRecWidth(), conf.getRectangleContur(), h1, h2, l1, l2, l3, l4, type, conf.getRoofTextureData());
    }
}
