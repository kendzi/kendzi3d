...
/**
 * Util for connecting set of adges ingo closed polygons.
 * 
 * @author Tomasz Kedziora (Kendzi)
 * 
 */
public final class MultiPartPolygonUtil {

    private MultiPartPolygonUtil() {
        //
    }
    ...
    private static <E, V> void markUsedForNotConnected(List<Edge<E, V>> all, Map<Vertex<V>, List<Connection<E, V>>> connections,
            Set<Edge<E, V>> used) {
    ...
    private static <E, V> List<List<EdgeOut<E, V>>> findClosedPolygon(List<Edge<E, V>> all,
        ...
        for (Edge<E, V> e : all) {
            ...
            if (connections.get(e.v1).size() < 2) {
    ...
    private static <E, V> boolean removeVertexWithOneEdge(Map<Vertex<V>, List<Connection<E, V>>> connections) {
        ...
        for (Vertex<V> vBegin : new HashSet<Vertex<V>>(connections.keySet())) {
            ...
            if (list.size() == 1) {
                ...
                for (int i = listEnd.size() - 1; i >= 0; i--) {
                    ...
                    if (vEnd.equals(connection.getBegin()) && vBegin.equals(connection.getEnd())) {
