    ...
    public static <W> W getRequiredContextVariable(String variableName, Context context, Class<W> class1) {
        ...
        if (obj == null) {
            throw new RuntimeException(
                    String.format("can't take required variable %s of type %s from context", variableName, class1));

        }
        if (!class1.isAssignableFrom(obj.getClass())) {
            throw new RuntimeException(
                    String.format("can't take required variable %s of type %s from context, wrong class", variableName, class1));
        }
